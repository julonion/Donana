...
# Variante E – OHNE DGM (nur spektrale Features)
# ===========================================
...
# Eingabedateien
samples_csv <- file.path(work_dir, "02_daten", "training", "samples_corine_2018_variante_e.csv")

# Ausgabedateien
model_output <- file.path(work_dir, "02_daten", "modelle", "rf_model_corine_only_variante_e.rds")
metrics_output <- file.path(work_dir, "03_ergebnisse", "interne_validierung", "rf_corine_only_variante_e_metrics.txt")
feature_list_path <- file.path(work_dir, "02_daten", "modelle", "rf_corine_only_variante_e_features.rds")
...
cat("===========================================\n")
cat("Random Forest Training (CORINE only)\n")
cat("Variante E – OHNE DGM (nur spektrale Features)\n")
cat("===========================================\n\n")

# === Prüfungen ===
if (!file.exists(samples_csv)) {
  stop("Trainingsdaten nicht gefunden. Bitte zuerst '03_extract_training_corine_only.R' ausführen!")
}
...
sink(metrics_output)
cat("Random Forest Training (CORINE only, Variante E)\n")
cat("================================================\n\n")
